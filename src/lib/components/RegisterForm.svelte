...
>
	<FieldGroup>
		<Field>
			<FieldLabel
				>Name
				<Input type="text" name="name" placeholder="Jane Doe" bind:value={name} required />
			</FieldLabel>
		</Field>
		...
		<Field>
			...
			{#if confirmPassword && !passwordsMatch}
				<FieldDescription class="mt-1 text-sm text-red-500">Passwords do not match.</FieldDescription>
			{/if}
		</Field>

		<Field>
			<Button type="submit" class="w-1/2" variant="secondary" disabled={isLoading || !passwordsMatch}>
			...
			{/if}
			{#if successMessage}
				<FieldDescription class="mt-2 text-center text-green-500">
					{successMessage}
				</FieldDescription>
			{/if}
		</Field>
