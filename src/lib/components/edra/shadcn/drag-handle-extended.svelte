				...
				<DropdownMenu.Sub>
					...
					<DropdownMenu.SubContent class="max-h-96 overflow-auto duration-300">
						{#each turnIntoCommand as command (command)}
							...
							<DropdownMenu.Item
								onclick={() => {
									if (currentNode && currentNodePos) command.turnInto?.(editor, currentNode, currentNodePos);
								}}
							>
								<Icon />
								<span>{command.tooltip}</span>
								<DropdownMenu.Shortcut class="bg-background rounded p-0.5 border"
						...
						{#each quickcolors as color (color.label)}
							<DropdownMenu.Item
								...
								onclick={() => {
									...
									else editor.chain().setNodeSelection(currentNodePos).setHighlight({ color: color.value }).run();
								}}
							>
								<span class="size-4 rounded-full border" style={`background-color: ${color.value};`}></span>
								<span class="capitalize">{color.label}</span>
							</DropdownMenu.Item>
						{/each}
					</DropdownMenu.Group>
			...
			<DropdownMenu.Sub>
				...
				<DropdownMenu.SubContent>
					...
					{#each alignments as alignment (alignment)}
						...
						<DropdownMenu.Item
							onclick={() => {
								if (currentNode && currentNodePos) alignment.turnInto?.(editor, currentNode, currentNodePos);
							}}
						>
							<Icon />
							{alignment.tooltip}
